...
### Security
- **OAuth ceremony hardened ahead of gating `/mcp` on the bearer token.** The `/authorize` endpoint now validates `redirect_uri` before issuing a code — dangerous schemes (`javascript:`/`data:`/non-loopback `http:`) are rejected, and a known client may only be redirected to a `redirect_uri` it registered (closes the open-redirector); PKCE is **S256-only** (`plain` is rejected and dropped from the advertised metadata); the issued authorization code is **bound to its `client_id`** at the token endpoint; and the `state` parameter is echoed back verbatim whenever sent (was dropped when empty). `/mcp` remains anonymous today, but the ceremony now issues bearer tokens safe to trust once gating lands.
...
### Fixed
- **Hygiene + accuracy (from the deep review):**
  - User-facing caveats in `rbi_check_compliance`, `rbi_search`, and `rbi_get_document` no longer claim the corpus is "limited to Master Directions" — they now name the actual coverage (Master Directions, Standalone Circulars, Master Circulars, Press Releases, FAQs) and drop stale `v0.1.5`/`v0.5` version tags.
  - `datetime.utcnow()` (deprecated, slated for removal — fires on Python 3.13) replaced with a timezone-aware `iso_utc_now()` helper across all 20 sites, preserving the exact `…Z` timestamp format stored in the corpus.
  - `rbi-source-fetch-corpus`: a disk-full / permission error during the final cross-filesystem copy or swap now routes to the structured PROBLEM/CAUSE/FIX abort (cleaning up the stray `.partial`) instead of escaping as a raw traceback; the existing corpus is left untouched.
  - `rbi-source-doctor`: the corpus check opens the DB read-only (consistent with every other reader, no WAL sidecars on a packaged file), and the model-cache check is guarded with `is_dir()` so a corrupt HF cache (a file where a dir is expected) no longer crashes the doctor.
  - Hybrid-search RRF tie-break is now a stable sort on fusion score (ties keep sparse-before-dense insertion order) rather than an arbitrary `chunk_id`-descending lexical bias. Corrected stale doc comments (`chunks_vec` is 768-dim not 384; `escape_fts5_query` OR-joins tokens, the docstring said AND).
- **MCP tool contract (from the deep review):**
  - `rbi_search` now surfaces a `filter_warning` when the declared-but-unimplemented `regulated_entity` / `as_of_date` filters are passed, instead of silently ignoring them (a silent no-op on a compliance/time-scoping filter is a wrong-results trap — matches `rbi_get_document`'s `as_of` caveat behavior).
  - The per-call `limit` is clamped to `[1, 50]` in the handler, so a negative `limit` no longer slices oddly and a huge one no longer wastes the embedder/DB.
  - Telemetry only records `topic_hint` when it's a recognized alias (else a constant `"other"`) — the field is free text in the schema, and the prior code captured it verbatim under a comment that wrongly claimed a closed enum. Keeps telemetry shape-only with bounded cardinality.
...
### Changed
- `README.md`: the **default quick-start is now the hosted MCP endpoint** (zero-install — paste `https://rbi-source.harshil.ai/mcp/` into any client). The local `pip install` path is reframed as the self-host alternative under a "Run locally (self-host)" section, and the connect table is split into Hosted / Self-host columns. Reflects that the lowest-friction path for most users is the remote MCP; self-hosting stays fully documented and first-class. No code change.
...
- **Cloudflare embedding creds are now read from `$CF_ACCOUNT_ID` + `$CF_API_TOKEN` only.** The undocumented `~/.gstack/cloudflare.json` file fallback was removed (env vars were always the documented path, and the default `local` provider needs no creds). Only affects `RBI_EMBEDDING_PROVIDER=cloudflare` users who relied on the file; `rbi-source-doctor` now points only at the env vars.
- Public-readiness doc pass: corrected the local embedding model in `pyproject.toml` + `CONTRIBUTING.md` (it's `bge-base-en-v1.5`, ~440 MB — not bge-small/~135 MB); widened the PyPI `description` to all content families; fixed the compressed-corpus download size in the README (~140 MB); pointed HTTP-change contributors at the in-repo `tests/test_http_*` instead of a script that lives out-of-tree. CI now also runs on Python 3.13 (matches the classifier). Removed internal maintainer-tooling references (gstack paths, a skill-routing section, slash-command names) from `CLAUDE.md` and code comments.
